...
def main() -> None:
    ...
    * Parses arguments
    ...
    if platform.system().lower() == "windows":
        print("")
        FSLError(
            "The required FSL dependencies cannot be installed on Windows platforms."
        )
    ...
    if not which("ss3t_csd_beta1"):
        print("")
        MRtrixError(
            "The required software (MRtrix, SS3T-beta) is not installed or on the system path."
        )

    parser = argparse.ArgumentParser(
        description="Constructs a structural connectome given a diffusion weighted image file, and an integer labeled atlas. NOTE: absolute paths must be passed as arguments."
    )
        ...
        try:
            if os.path.exists(in_file.file):
                out_file: str = os.path.join(
                    args.out_dir, os.path.basename(in_file.file)
                )
...
# Functions
def bias_corr(infile: str, outdir: str, log: LogFile = None) -> Tuple[str, str]:
...
class ReconMRtrix(object):
    ...
    class Mif(File):
        ...
        def __init__(self, file: File, gzip: bool = False) -> None:
    ...
    def dwi_nifti_to_mif(self, force: bool = False, gzip: bool = False) -> Mif:
    ...
    def nifti_to_mif(self, nii: NiiFile, force: bool = False) -> Mif:
    ...
    def mif_to_nifti(self, mif: Mif, force: bool = False):
        """General purpose conversion function for converting
    ...
    def mr_upsample(
        self, mif: Mif, vox: float, interp: Optional[str] = "cubic", gzip: bool = False
    ) -> Mif:
    ...
    def create_mask(
        ...
        tmp_b0s: TmpFile = work_dir.TmpFile(
            tmp_file="tmp_B0s" + _ext, tmp_dir=work_dir.tmp_dir
        )

        tmp_b0: TmpFile = work_dir.TmpFile(
            tmp_file="tmp_B0" + _ext, tmp_dir=work_dir.tmp_dir
        )

        tmp_mask: TmpFile = work_dir.TmpFile(
            tmp_file=tmp_b0.rm_ext(ext=_ext) + "_brain" + _ext, tmp_dir=work_dir.tmp_dir
        )
    ...
    def bias_field_correction(
        self, wm_fod: Mif, gm_tis: Mif, csf_tis: Mif, mask: Mif, gzip: bool = False
    ) -> Tuple[Mif, Mif, Mif]:
    ...
    def mr_tck_global(
        self, wm_fod: Mif, mask: Mif, stream_lines: int = 100000, cutoff: float = 0.07
    ) -> File:
        ...
        # Create output filenames
        tck: str = os.path.join(
            path, filename + f".{int(stream_lines)}.streamlines" + ".tck"
        )
        tck: File = File(tck)
    ...
    def mr_tck_sift(
        self, tck: File, wm_fod: Mif, term: Optional[int] = None, mask: Mif = ""
    ) -> File:
        ...
        # Create output filenames
        if term:
            tck_filt: str = os.path.join(
                path, filename + f".{int(term)}.streamlines.filtered" + ".tck"
            )
            tck_filt: File = File(tck_filt)
        else:
            tck_filt: str = os.path.join(
                path, filename + ".streamlines.filtered" + ".tck"
            )
            tck_filt: File = File(tck_filt)
    ...
    def structural_connectome(
        ...
        # Create output filenames
        connectome: str = os.path.join(
            path, filename + f".structural_connectome" + ".txt"
        )
        fa_connectome: str = os.path.join(
            path, filename + f".structural_connectome.fa_weighted" + ".txt"
        )
        md_connectome: str = os.path.join(
            path, filename + f".structural_connectome.md_weighted" + ".txt"
        )
        ad_connectome: str = os.path.join(
            path, filename + f".structural_connectome.ad_weighted" + ".txt"
        )
        rd_connectome: str = os.path.join(
            path, filename + f".structural_connectome.rd_weighted" + ".txt"
        )
        ...
        if fa or md or ad or rd:
            if dwi:
                [fa_mif, md_mif, ad_mif, rd_mif] = self.compute_diff_metrics(
                    dwi=dwi,
                    mask=mask,
                    fa=fa,
                    md=md,
...
class DWIxfm(object):
    ...
    def mask_dwi(
        self, frac_int: float = 0.5, bias_correct: bool = False
        ...
        [mask_mif, brain_mif, head_mif] = dwi.create_mask(
            mif=mif_file, frac_int=frac_int, bias_correct=bias_correct
        )
    ...
    def compute_linear_xfm(
        self, dwi_brain: NiiFile, dof: int = 12
    ) -> Tuple[File, NiiFile]:
    ...
    def compute_non_linear_xfm(
        self, xfm_mat: File, dwi_head: NiiFile
    ) -> Tuple[NiiFile, NiiFile, NiiFile]:
        ...
        nl_wpcf: str = os.path.join(
            path, filename + f".non-lin_xfm.warp_field_coeff" + ext
        )
...
def compute_xfm(
    ...
    [xfm_mat, xfm_out] = dwi.compute_linear_xfm(dwi_brain=brain, dof=12)

    # Compute non-linear transform
    [nl_out, nl_warp, nl_wpcf] = dwi.compute_non_linear_xfm(
        xfm_mat=xfm_mat, dwi_head=head
    )

    # Apply non-linear transform to atlas/template labels
    labels_native = dwi.applywarp(
        dwi_head=head, non_lin_warp=nl_warp, interp="nn", rel=True, premat=None
    )
    return labels_native, nl_out


def create_structural_connectome(
    dwi: str,
    bval: str,
    bvec: str,
    template: str,
    ...
    [labels_native, non_lin_out] = compute_xfm(
        dwi=f_tmp.file,
        bval=b_tmp.file,
        bvec=e_tmp.file,
        template=template,
    ...
    dwi_miff = mr_diff.dwi_nifti_to_mif(force=force, gzip=gzip)

    # Estimate response function(s)
    [wm_res, gm_res, csf_res] = mr_diff.estimate_response(
        mif=dwi_miff, erode=erode, fa_thresh=fa_thresh, force=force
    )

    # Upsample dwi and native space labels
    up_dwi_mif = mr_diff.mr_upsample(mif=dwi_miff, vox=vox)

    up_labels_native = mr_diff.mr_upsample(mif=labels_native, vox=vox, interp="nearest")

    # Perform brain extraction of B0s
    [mask, brain, head] = mr_diff.create_mask(
        mif=up_dwi_mif, frac_int=frac_int, cleanup=True, bias_correct=bias_correct
    )

    # Compute single-shell 3-tissue CSD
    [wm_fod, gm, csf] = mr_diff.ss3t_csd(
        mif=up_dwi_mif, mask=mask, wm_res=wm_res, gm_res=gm_res, csf_res=csf_res
    )

    # Perform joint bias-field correction
    [wm_fod_norm, gm_norm, csf_norm] = mr_diff.bias_field_correction(
        wm_fod=wm_fod, gm_tis=gm, csf_tis=csf, mask=mask
    )

    # Compute DEC and VF maps [for QC purposes]
    [dec, vf] = mr_diff.compute_dec_map(
        wm_fod=wm_fod_norm, gm_tis=gm_norm, csf_tis=csf_norm, mask=mask
    )

    # Reconstruct Fiber tracts
    if filter_tracts:
        unfilt_tcks = mr_diff.mr_tck_global(
            wm_fod=wm_fod_norm, mask=mask, stream_lines=stream_lines, cutoff=cutoff
        )
        tcks = mr_diff.mr_tck_sift(
            tck=unfilt_tcks, wm_fod=wm_fod_norm, term=term, mask=mask
        )
    else:
        unfilt_tcks = None
        tcks = mr_diff.mr_tck_global(
            wm_fod=wm_fod_norm, mask=mask, stream_lines=stream_lines, cutoff=cutoff
        )
...
def xfm_qc_fig(bg: NiiFile, overlay: NiiFile, outline: bool = False) -> File:
